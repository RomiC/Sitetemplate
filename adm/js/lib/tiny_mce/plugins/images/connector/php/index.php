...
//		var_dump(DIR_ROOT);
	...
	function Tree($beginFolder) {
		...
		if ($handle) {
			$struct[$beginFolder]['path'] = str_replace($this->dir['images'],'',$beginFolder);
			$tmp = preg_split('[\\/]',$beginFolder);
			$tmp = array_filter($tmp);
			end($tmp);
			$struct[$beginFolder]['name'] = current($tmp);
			$struct[$beginFolder]['count'] = -1;
			while (false !== ($file = readdir($handle))) {
				if ($file != "." && $file != ".." && $file != '.thumbs' && $file != 'temp') {
	...
	function UploadFile($dir, $type) {
		...
		//Файлы из обычной загрузки 
			...
			foreach ($ufiles['name'] as $k=>$v) {
				...
				if($pathtype == 'files') $allowed = $this->ALLOWED_FILES;
				else $allowed = $this->ALLOWED_IMAGES; 
